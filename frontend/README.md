...
This writes `openapi.json` at the repo root from the running container, **copies** it to `frontend/openapi.json`, then runs `openapi-ts` against that file (no live `localhost:8000` fetch) and `bun run lint` for the frontend workspace.
