...
def linkedin_authorize(current_user: CurrentUser) -> dict[str, str]:
    ...
    try:
        ...
        # fallback to in-memory in dev if redis unavailable
...
async def linkedin_callback(
    ...
    - Tokens are stored server-side (in-memory for now).
    """
    if not code or not state:
        redirect_to = (
            f"{settings.FRONTEND_HOST.rstrip('/')}/social-accounts?linkedin=error"
        )
