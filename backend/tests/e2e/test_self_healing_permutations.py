...
# ==============================================================================
# 3. DISK & IN-MEMORY CACHE PERMUTATIONS
# ==============================================================================
...
async def test_e2e_permutation_in_memory_fast_path_cache_hit(tmp_path: Path) -> None:
    ...
    with patch(
        ...
        # Call 1: Slow path (Heals via LLM)
        ...
        # Call 2: Fast path (Hit in-memory cache directly, 0 LLM calls)
        ...
        # LLM was NOT invoked a second time!
        assert mock_structured_model.ainvoke.await_count == 1


# ==============================================================================
# 4. MULTI-STEP RE-ENTRANT WORKFLOW PERMUTATIONS
# ==============================================================================
...
async def test_e2e_permutation_multi_step_publishing_healing_cascade(
    ...
    with (
        ...
        # Step 1: Enter compose text (Heals post_input)
        ...
        # Step 2: Submit post (Heals post_button)
        ...
        assert (
            selectors_dict["compose"]["post_button"]
            == "button[data-testid='tweetButtonInline']"
        )

        # Verify disk updated for both selectors
        with open(config_file) as f:
            persisted = json.load(f)
        assert (
            persisted["compose"]["post_input"] == "div[data-testid='tweetTextarea_0']"
        )
        assert (
            persisted["compose"]["post_button"]
            == "button[data-testid='tweetButtonInline']"
        )
