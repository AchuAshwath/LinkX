...
1. **Redirect URI** – In your backend `.env`, set `LINKEDIN_REDIRECT_URI` to exactly what you added in the LinkedIn Developer Portal (Auth → Redirect URLs). No trailing slash. Examples:
   - Dev: `http://localhost:8000/api/v1/auth/linkedin/callback`
   - Prod: `https://your-api-domain.com/api/v1/auth/linkedin/callback`
2. **Products** – In the Developer Portal → Products, request and get approved:
   - **Sign In with LinkedIn using OpenID Connect** (openid, profile, email)
   - **Share on LinkedIn** (w_member_social)
3. **Scopes** – Backend uses `LINKEDIN_SCOPES=openid profile email w_member_social`. Do not remove scopes your app does not have access to.
4. **Frontend redirect** – After OAuth, users are sent to `FRONTEND_HOST/social-accounts`. Set `FRONTEND_HOST` in backend `.env` (e.g. `http://localhost:5173` for dev).

### "Invalid redirect_uri"
- Ensure the redirect URI exactly matches what's configured in the Developer Portal
- Check for trailing slashes (LinkX normalizes by stripping them)
